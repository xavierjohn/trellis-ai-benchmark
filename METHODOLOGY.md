...
| **Adoption ≠ idiomatic use** — a "with-Trellis" run might scaffold the template but write largely plain C#, so the arm wouldn't actually exercise the framework. | **Real, and caught.** The neutral rubric (correctly) scores outcomes, not idioms, so a working-but-anemic Trellis service still scores well — which means the rubric *cannot* police whether the framework was actually used. We therefore added a **non-scored** idiom-usage diagnostic ([`harness/idiom_check.py`](harness/idiom_check.py)) written into every with-Trellis `meta.json` under `trellis_idiom_diagnostic`: it classifies a run `idiomatic` / `partial` / `template-only` by whether it uses the framework's load-bearing surfaces (value objects, smart enums / state machine, the MediatR pipeline) — `Result<T>` alone does **not** qualify. Result: **9 of 10** with-Trellis services (all three opus, all three sonnet, gpt runs 2–3, and the gpt validation run) are `idiomatic`; **`with-trellis/gpt-5.5/run-1` was `template-only`** — it kept `Result<T>` validation and the EF conventions but used a plain `enum`, raw primitives, no value objects, no aggregates/state machine, and no MediatR (controllers hit `DbContext` directly). Because that run does not satisfy the *condition* (build **with** the framework) — a **validity** problem, independent of its 30/30 score — it is being **re-generated**; the diagnostic is what made the gap auditable rather than invisible. This never touches the neutral score. |
