...
| **Cross-session leakage via shared memory / global instructions** — the clean-room isolates the *filesystem*, not the account's *memory*. | Copilot **user-scoped memories** (and global instruction files under `$HOME/.copilot/`) load into *every* session for the same user — including clean-room generations — and a generation session can even *write* memories that load into later ones (one with-Trellis run reported storing a fact about Trellis's 422 binder seam). **Empirically confirmed:** the de-named `without-trellis/sonnet-4.6/run-2` session's two user turns contained no "Trellis", yet the model referenced "the Trellis version" and "your benchmark" — and a brand-new benchmark cannot be in training data, so that knowledge came from loaded session context. **Impact is awareness-level, not code-copying:** the filesystem clean-room held, the artifacts are verified Trellis-free (0 mentions; the run made the *same* E3 mistake as the prompt-primed run, i.e. no measurable improvement), and the direction is conservative (awareness can only strengthen the baseline). **Mitigation:** memory is disabled (`/memory`) for the remaining generations and global instructions are checked; the 9 completed runs (awareness-level only) are kept and disclosed rather than re-generated. |
