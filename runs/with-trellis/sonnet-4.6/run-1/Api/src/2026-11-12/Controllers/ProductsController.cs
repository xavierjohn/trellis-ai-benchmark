    ...
    public ValueTask<ActionResult<ProductResponse>> Create([FromBody] CreateProductRequest request, CancellationToken cancellationToken) =>
        _sender.Send(new CreateProductCommand(request.Name, request.Sku, request.UnitPrice), cancellationToken)
            .ToHttpResponseAsync(
                ...
                options => options
                    .Created(product => $"/api/products/{product.Id.Value}?api-version=2026-11-12")
                    .WithETag(product => EntityTagValue.Strong(product.ETag))
                    .WithLastModified(product => product.LastModified))
