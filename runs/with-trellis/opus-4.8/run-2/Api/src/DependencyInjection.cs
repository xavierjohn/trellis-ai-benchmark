...
internal static class DependencyInjection
{
    public static IServiceCollection AddPresentation(this IServiceCollection services, IHostEnvironment environment)
    {
        ...
        {
            // Spec §9: validation failures (bad input, invalid state transition, insufficient
            // stock) map to 400 Bad Request. Trellis defaults these to 422, so remap globally.
            options.MapError<Trellis.Error.InvalidInput>(StatusCodes.Status400BadRequest);
            options.MapError<Trellis.Error.InvariantViolation>(StatusCodes.Status400BadRequest);
        });
        ...
        if (environment.IsDevelopment())
            ...
            {
                // Spec §5.5: when no X-Test-Actor header is present, default to an Admin actor.
                opts.DefaultActorId = "admin";
                opts.DefaultPermissions = new HashSet<string>(Permissions.All);
            });
        else
