...
internal static class DependencyInjection
{
    public static IServiceCollection AddPresentation(this IServiceCollection services, IHostEnvironment environment)
    {
        ...
        services.AddScoped<IActorProvider, TestHeaderActorProvider>();
...
{
    private const string HeaderName = "X-Test-Actor";
    private static readonly JsonSerializerOptions JsonOptions = new() { PropertyNameCaseInsensitive = true };
    private readonly IHttpContextAccessor _httpContextAccessor;

    public TestHeaderActorProvider(IHttpContextAccessor httpContextAccessor) =>
    ...
    public Task<Maybe<Actor>> GetCurrentActorAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (httpContext.Request.Headers.TryGetValue(HeaderName, out var header) && header.Count > 0)
        {
            var payload = JsonSerializer.Deserialize<TestActorPayload>(
                header.ToString(),
                JsonOptions);
            if (payload?.Id is { Length: > 0 } id)
                return Task.FromResult(Maybe.From(Actor.Create(id, payload.Permissions?.ToHashSet(StringComparer.Ordinal) ?? [])));
        }

        if (httpContext.User.Identity?.IsAuthenticated == true)
        {
            var id = httpContext.User.FindFirst("sub")?.Value ?? httpContext.User.FindFirst("oid")?.Value;
            if (!string.IsNullOrWhiteSpace(id))
            {
                var permissions = httpContext.User.FindAll("role").Select(c => c.Value).ToHashSet(StringComparer.Ordinal);
